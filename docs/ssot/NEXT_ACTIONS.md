...
| # | 質問 | 決定 | 理由 |
|---|------|------|------|
| Q1 | 既存 SSOT_*.md の変換方針 | **b) ハイブリッド** | 既存維持、新規のみ12セクション形式。既存SSOTにはフレームワーク以上の内容（Hard Gate等）があり完全変換で劣化リスク |
| Q2 | docs/ ディレクトリ構造 | **c) 段階的に移行** | ssot/ → core/ → 順次移動。一括移動はリンク切れリスク大 |
| Q3 | CLAUDE.md と .cursorrules | **c) 両方維持（用途別）** | Cursor + Claude Code 併用前提。CLAUDE.md = 仕様書ナビ、.cursorrules = ガードレール |
| Q4 | RFC 2119 用語 | **b) 英語（MUST/SHOULD/MAY）** | フレームワーク標準と一致。開発文脈で意味のブレが少ない |

> 上記は 2026-02-03 にプロダクトオーナーとの対話で確定済み。
...
| 2026-02-03 | Q1-Q4 意思決定事項をPOとの対話で確定 | AI（Claude Code） |
