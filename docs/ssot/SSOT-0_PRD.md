...
| ID | ユースケース | ユーザー | 優先度 |
|----|------------|---------|-------|
| UC-001 | 週間スケジュールの閲覧・編集 | ADMIN, LEADER, MEMBER | P0 |
| UC-002 | サイネージでの予定表示 | DEVICE | P0 |
| UC-003 | Googleカレンダー双方向同期 | ADMIN, LEADER, MEMBER | P0 |
| UC-004 | ユーザー・部門の管理 | ADMIN | P0 |
| UC-005 | AI日程調整（会議スロット提案） | ADMIN, LEADER | P1 |
| UC-006 | オフラインキオスク表示 | DEVICE | P1 |
| UC-007 | 多言語切り替え | 全ユーザー | P1 |
| UC-008 | デバイスログイン（サイネージ専用） | DEVICE | P0 |
...
- [x] 週間スケジュールボード（CRUD + リアルタイム同期）
- [x] サイネージ表示（大画面対応、自動更新）
- [x] ユーザー管理（CRUD, ロール: ADMIN/LEADER/MEMBER/DEVICE）
- [x] 部門管理（CRUD、ソフトデリート）
- [x] メール/パスワード認証 + デバイスログイン
- [x] Googleカレンダー双方向同期（OAuth2, Webhook, 増分同期）
- [x] AI日程調整（候補スロット生成、回答収集、確定）
- [ ] 多言語対応（7言語）
- [ ] オフラインキオスク（PWA + IndexedDB 24h キャッシュ）
...
| 指標 | 現状値 | 目標値 | 計測方法 |
|------|-------|-------|---------|
| 週間アクティブ組織数 | 0 | 10組織 | DB集計（Phase 0終了時） |
| ユーザー登録数 | 0 | 100名 | DB集計 |
| サイネージ稼働率 | - | 99.5% | ヘルスチェックAPI |
| リアルタイム同期遅延 | - | P95 < 1秒 | Socket.IO メトリクス |
| Googleカレンダー同期成功率 | - | 99% | ログ集計 |
| Lighthouse PWAスコア | - | 80以上 | Lighthouse CI |
