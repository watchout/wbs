# Test Strategy - テスト戦略

**プロジェクト**: ミエルボード for 現場  
**最終更新**: 2025-12-07

---

## 🎯 目的

「どこまでテストを書くか」を明確にし、**事業リスクが高い層**に集中的にテストを配置します。

---

## 📊 テスト優先度マトリクス

### 🔴 必須レベル（テスト必須）

**ビジネスクリティカルな領域 = バグが直接事業リスクになる箇所**

| 領域 | 理由 | テスト種別 |
|------|------|-----------|
| **認証・認可** | 他テナントのデータ漏洩リスク | ユニット + E2E |
| **マルチテナント境界** | organizationId分離の破綻 = 致命的 | ユニット + 統合 |
| **金銭計算・プラン判定** | 課金ミス = 信頼喪失 | ユニット |
| **Schedule CRUD** | コアビジネスロジック | 統合テスト |

#### 具体例

```typescript
// ✅ 必須: マルチテナント境界チェック
describe('GET /api/schedules', () => {
  it('should return only schedules for the authenticated organization', async () => {
    // テナントAのユーザーでログイン
    const responseA = await getSchedules(userA);
    expect(responseA.every(s => s.organizationId === 'org-a')).toBe(true);
    
    // テナントBのスケジュールが含まれていないことを確認
    expect(responseA.some(s => s.organizationId === 'org-b')).toBe(false);
  });
});
```

---

### 🟡 推奨レベル（あればベター）

**ロジックが複雑 or 将来の変更が多そうな箇所**

| 領域 | テスト種別 |
|------|-----------|
| **フォーマッター** (scheduleFormatter.ts など) | ユニット |
| **バリデーション** (入力検証ロジック) | ユニット |
| **UIコンポーネント** (Pure関数的な部分) | スナップショット |
| **Socket.IO イベント** | 統合テスト |

---

### 🟢 オプション（時間があれば）

| 領域 | 備考 |
|------|------|
| **静的ページ** | レイアウト崩れチェック程度 |
| **型定義のみのファイル** | TypeScriptで担保済み |

---

## 🛠️ テスト種別ごとの方針

### 1. ユニットテスト（Vitest）

**対象**: 純粋関数・ユーティリティ・ビジネスロジック

```typescript
// server/utils/scheduleFormatter.test.ts
import { formatScheduleForDisplay } from './scheduleFormatter';

describe('scheduleFormatter', () => {
  it('should format schedule with time and site name', () => {
    const result = formatScheduleForDisplay({
      start: '2025-01-15T09:00:00Z',
      end: '2025-01-15T18:00:00Z',
      metadata: { siteName: '◯◯ホテル', activityType: '工事' }
    });
    
    expect(result).toBe('9-18 ◯◯ホテル 工事');
  });
});
```

---

### 2. 統合テスト（API）

**対象**: API エンドポイント（認証・DB含む）

```typescript
// server/api/schedules/index.test.ts
import { createTestContext } from '~/tests/helpers';

describe('POST /api/schedules', () => {
  it('should create schedule with organizationId from auth context', async () => {
    const { user, orgId } = await createTestContext();
    
    const response = await $fetch('/api/schedules', {
      method: 'POST',
      body: { start: '2025-01-15T09:00:00Z', ... },
      headers: { Authorization: `Bearer ${user.token}` }
    });
    
    expect(response.organizationId).toBe(orgId);
  });
});
```

---

### 3. E2Eテスト（Playwright - Phase 1以降）

**対象**: クリティカルなユーザーフロー

- ログイン → 週間ボード表示 → スケジュール追加
- 他テナントのデータが見えないことの確認

---

## 📁 テストファイル配置ルール

```
server/
  api/
    schedules/
      index.get.ts
      index.get.test.ts  ← API横に配置
  utils/
    scheduleFormatter.ts
    scheduleFormatter.test.ts  ← ユーティリティ横に配置

components/
  WeeklyScheduleBoard.vue
  WeeklyScheduleBoard.test.ts  ← コンポーネント横に配置
```

---

## ✅ Definition of "Tested"（テスト済みの定義）

以下を満たすものを「テスト済み」とみなします：

- [ ] **必須レベル**の機能にはテストがある
- [ ] `npm run test` がグリーン
- [ ] カバレッジが主要ロジックで80%以上（目安）
- [ ] PRに「どのテストで何を検証したか」を記載

---

## 🚀 段階的導入プラン

### Phase 0（現在）
- 認証・マルチテナント境界のユニットテスト整備

### Phase 1
- Schedule CRUD の統合テスト追加
- フォーマッター・バリデーションのユニットテスト

### Phase 2
- E2Eテスト（Playwright）導入
- CIにカバレッジレポート追加

---

## 🔗 関連ファイル

- `vitest.config.ts` - Vitest設定
- `.github/workflows/ci.yml` - CI実行設定
- `docs/DONE_DEFINITION.md` - テスト完了の基準







