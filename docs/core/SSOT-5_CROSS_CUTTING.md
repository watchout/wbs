...
```
MUST: APIアクセス時にセッション有効期限をチェック
MUST: 延長時はCookieも同時に更新
SHOULD: 50%境界値は延長対象外（厳密に未満）
```
...
- [x] AC4: デバイスセッションも同様に動作
```
**実装ファイル**: `server/utils/session.ts`, `server/utils/authMiddleware.ts`
