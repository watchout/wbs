# SSOT: 共通ヘッダー・設定メニュー [DETAIL]

> 機能ID: FEAT-HDR-001
> ステータス: Implemented
> 最終更新: 2026-02-05
> 関連: SSOT-1 #ACCT-001〜003, SSOT-2
...
| 1 | 概要 | §1 背景・課題, §2 機能概要 |
| 2 | ユーザーストーリー | §2 機能概要 |
| 3 | 画面仕様 | §3〜8 各コンポーネント仕様 |
| 4 | API仕様 | SSOT-3参照 |
| 5 | データモデル | SSOT-4参照 |
| 6 | ビジネスロジック | §4 ユーザーメニュー |
| 7 | エラーハンドリング | SSOT-5参照 |
| 8 | セキュリティ | §6 管理メニュー（ADMINのみ） |
| 9 | パフォーマンス | §10 モバイル対応 |
| 10 | テストケース | §11 テスト要件 |
| 11 | 実装メモ | §9 実装ファイル一覧 |
| 12 | 未決事項 | §12 関連ドキュメント |  
...
| HDR-03-005 | カレンダー連携リンク（/settings/calendar） | ✅ 実装済み |
| HDR-03-006 | 設定一覧ページ（/settings） | ✅ 実装済み |
